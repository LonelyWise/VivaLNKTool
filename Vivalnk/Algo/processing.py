...
def get_filtered(x, y, z, fs_acc):
    n = 5
    if fs_acc == 250:
        b = np.array([0.13391353, 0, -0.26782706, 0, 0.13391353])
        a = np.array([1, -2.65789221, 2.62522042, -1.23388001, 0.26803863])
        zi = np.array([-0.13391353, -0.13391353, 0.13391353, 0.13391353])
    else:
        b = np.array([0.04221355, 0, -0.0844271, 0, 0.04221355])
        a = np.array([1, -3.31912288, 4.1541403, -2.34436531, 0.50946582])
        zi = np.array([-0.04221355, -0.04221355, 0.04221355, 0.04221355])

    # filter x,y,z components of accelerometer signal (must be magnified)
    x_filt = filtfilt(x, len(x), b, a, zi, n)
    y_filt = filtfilt(y, len(y), b, a, zi, n)
    z_filt = filtfilt(z, len(z), b, a, zi, n)
